...
const WebsiteEditor = () => {
  ...
  const handleDeploy = async () => {
    try {
      const result = await axios.get(
        `${serverUrl}/api/website/deployWebsite/${website._id}`,
        { withCredentials: true },
      );
  ...
  useEffect(() => {
    const handleGetWebsite = async () => {
      try {
        const result = await axios.get(`${serverUrl}/api/website/${id}`, {
          withCredentials: true,
        });
  ...
  return (
    <div className="h-screen w-screen flex bg-black text-white overflow-hidden">
        ...
        <>
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
            {message.map((m, i) => (
              ...
              >
                <div
                  className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${m.role === "user" ? "bg-white text-black" : "bg-white/5 border border-white/50 text-zinc-200"}`}
                >
                  {m.content}
                </div>
              </div>
          ...
          <div className="p-3 border-t border-white/10">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 resize-none rounded-2xl px-4 py-3 bg-white/5 border border-white/50 text-sm outline-none"
      ...
      <div className="flex-1 flex flex-col">
        <div className="h-14 px-4 flex justify-between items-center border-b border-white/60 bg-black/80">
          ...
          <div className="flex gap-2">
            {website.deployed ? (
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-1.5 rounded-lg bg-linear-to-r from-indigo-500 to-pink-500 text-sm font-semibold hover:scale-105 transition"
              >
      ...
      <AnimatePresence>
        {showChat && (
          <motion.div
            ...
            animate={{ x: 0 }}
            exit={{ x: "100%" }}
            className="fixed inset-y-0 z-[9999] bg-black flex flex-col"
          >
  ...
  function Header({ onclose }) {
    return (
      <div className="h-14 px-4 flex items-center justify-between border-b border-white/60">
