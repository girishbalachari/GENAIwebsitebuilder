...
const plans = [
  {
    ...
    credits: 100,
    description:
      "Sufficient to Generate few website for free using Website Builder",
    features: [
  ...
  },
  {
    ...
    credits: 500,
    description:
      "Professionals who want to Generate more website using Website Builder",
    features: [
  ...
  },
  {
    ...
    credits: 1000,
    description:
      "For teams who want to Generate more website using Website Builder",
    features: [
  ...
  const handleBuy = async (planKey) => {
    ...
    try {
      ...
      if (result.data.sessionUrl) {
        // window.location.href = result.data.sessionUrl; // Redirect to Stripe
      ...
      <div className="relative z-10 max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
        {plans.map((p, i) => (
          <motion.div
            ...
            className={`relative rounded-3xl p-8 border backdrop-blur-xl transition-all ${p.popular ? "border-indigo-500 bg-linear-gradient-to-b from -indigo-500/20 to-transparent shadow-2xl shadow-indigo-500/30" : "border-white/10 bg-white/5 hover:border-indigo-400 hover:bg-white/10"}`}
          >
            {p.popular && (
